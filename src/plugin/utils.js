...
const generateParams = (params: { [key: string]: any }) => {
  ...
  pagePath.forEach((item) => {
    const routerSplit = item.split('/').map((routerSplitItem) => routerSplitItem.charAt(0).toUpperCase() + routerSplitItem.slice(1));

    //根据是否是分包生成方法名称
    const methodName = routerSplit.length > 4 ? routerSplit[1] + routerSplit[3] : routerSplit[2];
...
export const to${methodName}Page = (option?: ToRouterType<${toRouterType}>) => {
  navigateType("${item}", option);
    ...
    const pathUrl = path.resolve(ctx.paths.sourcePath, `.${item.slice(0, item.lastIndexOf('/')) + '\\route.config.ts'}`);
    //判断文件是否存在
    if (ctx.helper.fs.existsSync(pathUrl)) {
      ...
      if (contentStr.includes('Params')) {
        importRouteParamsStr += `import ${methodName}Params from '..${item.slice(0, item.lastIndexOf('/')) + '/route.config'}'\n`;
